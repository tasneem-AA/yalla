"use client";

import { useEffect, useState } from "react";

type Download = {
  id: number;
  platform: "ios" | "android";
  createdAt: string;
};

export default function AdminDownloadsPage() {
  const [downloads, setDownloads] = useState<Download[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<"all" | "ios" | "android">("all");

  useEffect(() => {
    fetch("/api/downloads")
      .then((r) => r.json())
      .then((data) => {
        setDownloads(data.downloads ?? []);
        setLoading(false);
      })
      
  }, []); 
  
  const filtered = downloads.filter((d) => {
    const matchPlatform = filter === "all" || d.platform === filter;
    const matchSearch = !search || d.platform.toLowerCase().includes(search.toLowerCase());
    return matchPlatform && matchSearch;
  });

  const iosCount = downloads.filter((d) => d.platform === "ios").length;
  const androidCount = downloads.filter((d) => d.platform === "android").length;

  return (
    <>
      <h1 className="admin-page-title">Downloads</h1>
      <p className="admin-page-sub">All download events tracked by the app.</p>

      {/* Stats strip */}
      <div className="admin-kpi-row">
        <div className="kpi-card">
          <div className="kpi-label">Total</div>
          <div className="kpi-value" style={{ fontSize: 28 }}>{downloads.length}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">iOS</div>
          <div className="kpi-value" style={{ fontSize: 28, color: "var(--yellow)" }}>{iosCount}</div>
        </div>
        <div className="kpi-card">
          <div className="kpi-label">Android</div>
          <div className="kpi-value" style={{ fontSize: 28, color: "#4ade80" }}>{androidCount}</div>
        </div>
      </div>

     
      <div className="admin-search">
       
        
        {(["all", "ios", "android"] as const).map((f) => (
          <button
            key={f}
            className={`btn-sm ${filter === f ? "btn-sm-yellow" : "btn-sm-ghost"}`}
            onClick={() => setFilter(f)}
          >
             {f === "all" ? "" : f === "ios" ? (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ display: "inline", marginRight: 4 }}>
                        <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                      </svg>
                    ) : (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ display: "inline", marginRight: 4 }}>
                        <path d="M6 18c0 .55.45 1 1 1h1v3.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5V19h2v3.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5V19h1c.55 0 1-.45 1-1V8H6v10zM3.5 8C2.67 8 2 8.67 2 9.5v7c0 .83.67 1.5 1.5 1.5S5 17.33 5 16.5v-7C5 8.67 4.33 8 3.5 8zm17 0c-.83 0-1.5.67-1.5 1.5v7c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5v-7c0-.83-.67-1.5-1.5-1.5zm-4.97-5.84l1.3-1.3c.2-.2.2-.51 0-.71-.2-.2-.51-.2-.71 0l-1.48 1.48C13.85 1.23 12.95 1 12 1c-.96 0-1.86.23-2.66.63L7.85.15c-.2-.2-.51-.2-.71 0-.2.2-.2.51 0 .71l1.31 1.31C6.97 3.26 6 5.01 6 7h12c0-1.99-.97-3.75-2.47-4.84zM10 5H9V4h1v1zm5 0h-1V4h1v1z"/>
                      </svg>
                    )}
            {f === "all" ? "All" : f === "ios" ? (" iOS" ): (" Android")}
            
          </button>
        ))}
      </div>

      {/* Table */}
      <div className="admin-table-card">
        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: "var(--gray-3)" }}>Loading…</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: "var(--gray-3)" }}>No results found.</div>
        ) : (
          <table className="admin-table">
            <thead>
              <tr>
                <th>#</th>
                <th>Platform</th>
                <th>Date & Time</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((d) => (
                <tr key={d.id}>
                  <td style={{ color: "var(--gray-4)", fontFamily: "monospace" }}>{d.id}</td>
                  <td>
                    <span className={`badge ${d.platform === "ios" ? "badge-yellow" : "badge-green"}`}>
                        {d.platform === "ios" ? (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ display: "inline", marginRight: 4 }}>
                        <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                      </svg>
                    ) : (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ display: "inline", marginRight: 4 }}>
                        <path d="M6 18c0 .55.45 1 1 1h1v3.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5V19h2v3.5c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5V19h1c.55 0 1-.45 1-1V8H6v10zM3.5 8C2.67 8 2 8.67 2 9.5v7c0 .83.67 1.5 1.5 1.5S5 17.33 5 16.5v-7C5 8.67 4.33 8 3.5 8zm17 0c-.83 0-1.5.67-1.5 1.5v7c0 .83.67 1.5 1.5 1.5s1.5-.67 1.5-1.5v-7c0-.83-.67-1.5-1.5-1.5zm-4.97-5.84l1.3-1.3c.2-.2.2-.51 0-.71-.2-.2-.51-.2-.71 0l-1.48 1.48C13.85 1.23 12.95 1 12 1c-.96 0-1.86.23-2.66.63L7.85.15c-.2-.2-.51-.2-.71 0-.2.2-.2.51 0 .71l1.31 1.31C6.97 3.26 6 5.01 6 7h12c0-1.99-.97-3.75-2.47-4.84zM10 5H9V4h1v1zm5 0h-1V4h1v1z"/>
                      </svg>
                    )}{d.platform === "ios" ? " iOS" : " Android"}
                    </span>
                  </td>
                
                  <td style={{ color: "var(--gray-3)" }}>
                    {new Date(d.createdAt).toLocaleString("en-US", {
                      month: "short", day: "numeric",
                      hour: "2-digit", minute: "2-digit",
                    })}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>
    </>
  );
}
