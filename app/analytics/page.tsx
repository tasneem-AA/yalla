  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [downloadsRes, feedbackRes] = await Promise.all([
          fetch("/api/downloads"),
          fetch("/api/feedback"),
        ]);
        ...
        let downloads: any[] = downloadsData.downloads ?? [];
        const feedback: any[] = feedbackData.feedback ?? [];

        // ─── GENERATE MOCK DATA IF NO DOWNLOADS ───
        ...
        downloads.forEach((d: any) => {
          try {
            ...
            if (diffDays >= 0 && diffDays < 56) {
              ...
              const weekLabel = weekLabels[7 - weekIndex];
              if (weekLabel && weekLabel in weeklyData) {
        ...
        const weeklyDownloads = weekLabels.map((week) => ({
          week,
            ...
            <>
              <div className="analytics-kpi-grid">
                {[
                  ...
                  { label: "iOS downloads", value: stats.iosCount.toLocaleString() },
                  { label: "Android downloads", value: stats.androidCount.toLocaleString() },
                  { label: "Avg feedback rating", value: `${stats.avgRating.toFixed(1)}★` },
                  { label: "Feedback submissions", value: stats.totalFeedback.toLocaleString() },
                  {
                    label: "5-star feedback",
                    value: feedbackItems.filter((f) => f.rating === 5).length.toLocaleString(),
                  },
                ].map((k) => (
                      ...
                      return (
                        <div key={d.week} className="chart-bar-wrap">
                          <div
                            ...
                            style={{
                              height: `${heightPct}%`,
                              background: d.count > 0 ? "var(--yellow)" : "rgba(255,230,0,0.1)",
                          ...
                          <span style={{ fontSize: "8px", color: "var(--gray-4)" }}>{d.count}</span>
                        </div>
                      );
                    })}
                  </div>
